...
class Main extends PluginBase {
    ...
    public void onEnable() {
        ...
        getEventListener().subscribeAlways(GroupMessageEvent.class, (GroupMessageEvent e) -> {
            if (e.getMessage().contentToString().toLowerCase().equals("色图") || e.getMessage().contentToString().toLowerCase().equals("涩图")) {
                if (count > 15) {
            ...
            if (e.getMessage().contentToString().toLowerCase().equals("r18")) {
                if (count > 15) {
                    e.getGroup().sendMessage(MessageUtils.newChain(new At(e.getSender())).plus("一分钟内调用次数上限！"));
                    return;
                }
            ...
            if (e.getMessage().contentToString().toLowerCase().equals("混合")) {
                if (count > 15) {
                    e.getGroup().sendMessage(MessageUtils.newChain(new At(e.getSender())).plus("一分钟内调用次数上限！"));
                    return;
                }
